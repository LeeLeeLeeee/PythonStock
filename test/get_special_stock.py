...
class CpRpMarketWatch:
    ...
    def Request(self, code, caller):
        ...
        for i in range(cnt):
            ...
            caller.listWatchData.append(item) # 윈도우 화면 listWatchData
...
class MyWindow(QMainWindow):
    ...
    def btnExcel_clicked(self):
        ...
        writer = pd.ExcelWriter(gExcelFile, engine='xlsxwriter') #판다스 데이터 프레임으로 엑셀 생성
        # Convert the dataframe to an XlsxWriter Excel object.
        df.to_excel(writer, sheet_name='Sheet1') # 데이터 프레임 sheet1에 쓰기
        # Close the Pandas Excel writer and output the Excel file.
        writer.save() # 엑셀 저장
        os.startfile(gExcelFile) # 엑셀 실행
        return
